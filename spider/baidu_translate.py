...
class BaiduFanyi(object):
    ...
    def trans(self, lan):
        ...
        try:
            ...
            # print(response_dict['dict_result']['usecase']['idiom'][0]['data'])
            result = list()
            try:
                for d in response_dict['dict_result']['usecase']['idiom'][0]['data']:
                    item = d['data'][0]['enText']
                    result.append(item.strip())
            except Exception as e:
                print(e)
            duanyus = "|".join(result);
            str_file = self.keywords+"\t"+duanyus+"\n"
            f = open("Q:\\duanyu.txt", "a", encoding="UTF-8")
            f.write(str_file)
            f.close()
...
def main():
    for line in open("Q:\\watch\\en.txt", "r", encoding="UTF-8"):
        time.sleep(random.randint(1,4))
        baidu_fanyi = BaiduFanyi(line.strip())
        baidu_fanyi.run()
